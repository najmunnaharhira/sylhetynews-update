...
interface PhotocardTemplateRow extends RowDataPacket {
  id: number;
  name: string;
...
const serializeTemplate = (row: PhotocardTemplateRow) => ({
  id: String(row.id),
  name: row.name,
...
router.get('/', async (_req: Request, res: Response) => {
  try {
    const rows = await query<PhotocardTemplateRow[]>(
...
router.post('/', requireAdmin, async (req: Request, res: Response) => {
  try {
    const { name, description, imageUrl, previewUrl, category, isActive } = req.body as Record<string, unknown>;
...
router.put('/:id', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (typeof body.previewUrl === 'string') {
      ...
      values.push(body.previewUrl.trim());
    }
...
router.delete('/:id', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const result = await query<ResultSetHeader>(`DELETE FROM photocard_templates WHERE id = ?`, [
      templateId,
    ]);
